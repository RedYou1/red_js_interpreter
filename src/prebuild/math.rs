...
new_class! {
    ...
    abs, fn,
    |_, _, [num]| {
        Rc::new(RefCell::new(match inline_borrow!(num) {
    ...
    |_, _, arguments| {
        ...
        Rc::new(RefCell::new(if i64::MIN as f64 <= max && max <= i64::MAX as f64 && max.floor() == max { JsValue::BigInt(max as i64) } else {JsValue::Number(max)}))
    },
    min, fn_direct,
    |_, _, arguments| {
        let mut min = f64::MIN;
        ...
        Rc::new(RefCell::new(if i64::MIN as f64 <= min && min <= i64::MAX as f64 && min.floor() == min { JsValue::BigInt(min as i64) } else {JsValue::Number(min)}))
    },
    pow, fn,
    |_, _, [base, exponent]| {
